...
#     # Create a named window without explicit repositioning
...
#     # Create a figure with two subplots using the overall window size (8,5)
#     plt.ion()
#     fig, axs = plt.subplots(2, 1, figsize=(8, 5))
    
#     # Adjust the layout so that both subplots shrink to fit the window and labels are not cut off.
#     fig.subplots_adjust(top=0.85, bottom=0.15, left=0.1, right=0.95, hspace=0.4)
    
#     # Initialize plot lines and fixed axis limits.
...
def playVideo(label):
    ...
    # Optionally, set the video window to topmost so it's not hidden.
...
def main():
    ...
    # Create the Matplotlib figure with the original size.
    plt.ion()
    fig, axs = plt.subplots(2, 1, figsize=(10, 6))
    fig.subplots_adjust(top=0.85, bottom=0.15, left=0.1, right=0.95, hspace=0.4)
    
    # Reposition the Matplotlib window to cover the left half (960x1080) of a 1920x1080 screen.
